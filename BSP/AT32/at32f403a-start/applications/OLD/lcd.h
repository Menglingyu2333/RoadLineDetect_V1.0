...
#define LCD_BL_ON   GPIOC->BSRE=1<<5
#define LCD_BL_OFF  GPIOC->BRE=1<<5

//TFTLCD部分外要调用的函数
extern u16  POINT_COLOR;//默认红色
extern u16  BACK_COLOR; //背景颜色.默认为白色

#define	LCD_CS_SET  GPIOC->BSRE=1<<10    //片选端口  	PA2
...
#define	LCD_CS_CLR  GPIOC->BRE=1<<10     //片选端口  	PA2
...
#define DATAOUT(x) GPIOE->OPTDT=x; //printf("GPIOE->OPTDT=%04X\x00", GPIOE->IPTDT) //数据输出
...
void GUI_DrawPoint(u16 x,u16 y,u16 color);
u32 mypow(u8 m,u8 n);
void UART_Printf(char *str,rt_size_t size);
