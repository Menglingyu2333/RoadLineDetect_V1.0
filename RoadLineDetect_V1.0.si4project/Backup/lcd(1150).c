...
void LCD_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2PERIPH_GPIOC|RCC_APB2PERIPH_GPIOA|RCC_APB2PERIPH_GPIOE, ENABLE); //使能PORTB,C时钟和AFIO时钟
//    GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable , ENABLE);//开启SWD，失能JTAG

    GPIO_InitStructure.GPIO_Pins = GPIO_Pins_1;     ///PORTC6~10复用推挽输出
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT_PP;
    GPIO_InitStructure.GPIO_MaxSpeed = GPIO_MaxSpeed_50MHz;
    GPIO_Init(GPIOA, &GPIO_InitStructure); //GPIOE
    GPIO_SetBits(GPIOA,GPIO_Pins_1);
    GPIO_InitStructure.GPIO_Pins = (GPIO_Pins_0|GPIO_Pins_1|GPIO_Pins_2|GPIO_Pins_3|GPIO_Pins_5);     ///PORTC6~10复用推挽输出

    GPIO_SetBits(GPIOC,GPIO_Pins_0|GPIO_Pins_1|GPIO_Pins_2|GPIO_Pins_3|GPIO_Pins_5);

    GPIO_InitStructure.GPIO_Pins = GPIO_Pins_All; //  PORTB推挽输出
    ...
    sprintf(PrintStr, " LCD ID:%x\r\n\x00", lcddev.id);
    UART_Printf(PrintStr); //打印LCD ID
