...
{
  u16 RGB565Gray;
  RGB565Gray=((Gray<<11)|(Gray<<5)|Gray);
	return RGB565Gray;
}
u16 RGB565_to_Gray(u16 RGB)
{
  u16 Gray;
  unsigned int R  = (RGB & RGB565_RED)    >> 8;
  unsigned int G  = (RGB & RGB565_GREEN)  >> 3;
  unsigned int B  = (RGB & RGB565_BLUE)   << 3;
  Gray=(u16)(( R*38 +  G * 75 +  B * 15 )>>7);
  return Gray;
}
